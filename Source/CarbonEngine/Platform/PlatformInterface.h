...
/**
 * This is the base platform class which defines an interface for creating a rendering surface, resizing, fullscreen switching,
 * input state, timing, and is responsible for firing input events generated by the user. Actual implementations inherit from
 * this class and provide a backend for a specific platform.
 */
...
public:
    ...
    /**
     * Enum used to specify a full screen antialiasing mode. Not all FSAA modes will be available depending on the hardware
     * configuration. If a FSAA mode is requested that can't be done in hardware then the next highest available mode will be
     * used.
     */
    enum FSAAMode
    {
    ...
    /**
     * Sets the given platform-specific value. If the specified value is not supported or can't be set then false is returned.
     */
    virtual bool setPlatformSpecificValue(PlatformSpecificValue value, uintptr_t newValue) { return false; }

    /**
     * Returns whether this platform and display device support windowed mode rendering, if this is false then all rendering
     * will be done in fullscreen regardless of the requested window mode.
     */
     ...
     * possible then the fallback resolution is attempted. If this fails then the fullscreen setting is flipped and another
     * attempt is made. The previous fullscreen and FSAA settings are preserved if possible. Returns success flag.
     */
    bool createWindow();
    ...
    /**
     * Resizes the rendering window, takes the same parameters as PlatformInterface::createWindow(). Sends a ResizeEvent for the
     * new window size. Returns success flag.
     */
    ...
    /**
     * Sets the window title of the rendering window. The default title is the name of the application's main Application
     * subclass, however this can be changed using this method. Returns success flag.
     */
    ...
    /**
     * Templated variant of PlatformInterface::getOpenGLFunctionAddress() that does a reinterpret_cast internally to make
     * calling it less verbose.
     */
    ...
    /**
     * Sets whether vertical sync is enabled, vertical sync prevents tearing artifacts by synchronizing buffer swaps with
     * display updates, but will limit the frame rate to the refresh rate of the display. Returns success flag.
     */
    virtual bool setVerticalSyncEnabled(bool enabled) { return false; }

    /**
     * Returns the requested resolution. Resolutions are referenced by a unique ID and are sorted by size from smallest to
     * largest. If \a index is invalid then a zeroed Resolution object is returned. The valid range for resolution ID values is
     * from 1 to the number of resolutions returned by PlatformInterface::getResolutionCount().
     */
     ...
     */
    bool hasResolution(unsigned int width, unsigned int height) const { return findResolution(width, height).isValid(); }
    ...
    /**
     * Returns the current resolution for the render window, or Resolution::Zero if no window is active. The current dimensions
     * of the render window can be queried with PlatformInterface::getWindowWidth() and PlatformInterface::getWindowHeight().
     */
    ...
    /**
     * Sets the startup resolution and window mode to use. By default the startup resolution is the resolution that was in use
     * when the application was last shut down. If this is the first run of the application then the native resolution as
     * returned by PlatformInterface::getNativeResolution() will be used as the startup resolution. Note that if an application
     * wants to use this method to control the initial resolution then it should do so in its Application::startup() override as
     * that will be called prior to window creation, its Application::initialize() override is called following window creation
     * which is after the startup resolution will have been used. Returns success flag.
     */
     ...
     * PlatformInterface::areCustomResolutionsSupported() returns true. If the given resolution is already supported then
     * nothing is done. Custom resolutions allow arbitrary window dimensions to be used, however do note that they are generally
     * not compatible with fullscreen rendering and so may limit the application to running in windowed mode. Returns success
     * flag.
     */
     ...
     * Resolution::isRetinaResolution() are allowed to be used, and when it is false an attempt to create a window using a
     * retina resolution will result in a fallback to the matching non-retina resolution. Retina resolutions are enabled by
     * default. If an application wants to disable retina rendering then it should call
    ...
    /**
     * Sets whether retina resolutions are enabled, see \a PlatformInterface::areRetinaResolutionsEnabled() for details. Calling
     * this method does not change the current resolution, but will take effect on subsequent resolution changes.
     */
    ...
    /**
     * Sometimes the aspect ratio for the final displayed image is different to the aspect ratio of the primary rendering
     * surface, i.e. the final render is stretched for display. If this is the case then the final display aspect ratio returned
     * from this method will be the aspect ratio after any such stretching. The default implementation of this method returns
     * PlatformInterface::getWindowAspectRatio().
     */
    ...
    /**
     * Returns the current setting as to whether the main window should grab an exclusive lock on user input when it gains
     * focus. Defaults to true. The main window will always release any exclusive input lock it is holding when it loses focus.
     * Note that this only applies when in windowed mode, fullscreen rendering will always take an exclusive input lock
     * regardless of this value.
     */
    bool isInputLockEnabled() const { return isWindowedModeInputLockEnabled_; }

    /**
     * Sets whether the main window should grab an exclusive lock on user input when it gains focus. Note that setting this to
     * false will not release any current input lock, use the PlatformInterface::releaseInputLock() method to force that. This
     * method only applies when in windowed mode, fullscreen rendering will always take an exclusive input lock regardless of
    ...
    /**
     * Releases the main window's exclusive lock on user input if it is currently holding one. Any input lock is automatically
     * released when the main window loses focus, and retaken when it regains focus.
     */
    ...
    /**
     * Sets the current absolute mouse position, the given position is clamped inside the window bounds. This method will
     * dispatch a MouseMoveEvent if the new position is different to the previous position. Applications can use this method to
     * control the position of the mouse on platforms that don't have a default pointing device.
     */
    virtual void setMousePosition(const Vec2& position);

    /**
     * This is a small helper method that moves the mouse position by the specified delta, thus avoiding having to manually add
     * the delta onto the current mouse position.
     */
    ...
    /**
     * Sets whether all the mouse input events will be sent, internally this just calls EventManager::setEventAllowed() for each
     * of the four mouse input events: MouseButtonDownEvent, MouseButtonUpEvent, MouseMoveEvent and MouseWheelEvent.
     */
    void setMouseInputEventsAllowed(bool allowed);

    /**
     * Sets whether all the keyboard input events will be sent, internally this just calls EventManager::setEventAllowed() for
     * each of the three keyboard input events: KeyDownEvent, KeyUpEvent and CharacterInputEvent.
     */
    ...
    /**
     * Displays a custom message box and returns the button the user pressed, true means either OK or Yes was clicked and false
     * means either Cancel or No was clicked.
     */
    virtual bool showMessageBox(const UnicodeString& text, const UnicodeString& title, MessageBoxButtons buttons = OKButton,
                                MessageBoxIcon icon = InformationIcon)
    {
    ...
    /**
     * Refreshes the list of known game controllers, any controllers that have been added or removed from the system will appear
     * or disappear from the game controller list. The game controller list is refreshed automatically by
     * PlatformInterface::createWindow() and PlatformInterface::resizeWindow(). Calling this method invalidates all previously
     * existing game controller IDs.
     */
    virtual void refreshGameControllerList() {}

    /**
     * Clears all game controllers that are currently initialized. After this is called PlatformInterface::getGameControllers()
     * will return an empty vector and no game controllers will be able to be used until
     * PlatformInterface::refreshGameControllerList() is called again to reinitialize any game controllers attached to the
     * system. Clearing the game controller list is useful because it releases any internal lock on the device that may prevent
     * it being used by other applications. This method also erases any force feedback effects that have been created.
     */
     ...
     * invalid. If zero is passed in for the controller ID then the first game controller in the game controller list is used.
     */
    virtual UnicodeString getGameControllerDisplayName(unsigned int controllerID) { return UnicodeString::Empty; }

    /**
     * Returns the state of a game controller. If zero is passed in for the controller ID then the first game controller in the
     * game controller list is used. If \a controllerID is non-zero and is not a valid game controller ID or there are no game
     * controllers attached to the system then GameControllerState::Empty is returned.
     */
    virtual const GameControllerState& getGameControllerState(unsigned int controllerID) { return GameControllerState::Empty; }

    /**
     * Creates a force feedback effect for a game controller and returns the ID value for the new effect. If the device does not
     * support the given effect then zero is returned. If zero is passed in for the controller ID then the first game controller
     * in the game controller list is used.
     */
    virtual unsigned int createForceFeedbackEffect(unsigned int controllerID, const ForceFeedbackEffect& effect) { return 0; }

    /**
     * Clears a force feedback effect. If zero is passed in for the controller ID then the first game controller in the game
     * controller list is used.
     */
     ...
     * PlatformInterface::createForceFeedbackEffect() method. If zero is passed in for the controller ID then the first game
     * controller in the game controller list is used.
     */
    ...
    /**
     * Stops a force feedback effect that is currently playing on a game controller. If zero is passed in for the controller ID
     * then the first game controller in the game controller list is used.
     */
    ...
    /**
     * Returns the current gamma ramp values in use. A gamma ramp value of zero means the gamma curve present at application
     * startup is being used.
     */
    const Color& getGamma() const { return gammas_; }

    /**
     * Sets the current red, green and blue gamma values. A gamma of zero means the gamma curve present at application startup
     * should be used, which is the default. Returns success flag.
     */
    ...
    /**
     * Creates an input mapping from one key to another, i.e. when a raw '\a from' keypress is received it will be translated
     * into a '\a to' keypress before any further processing is done. Mapping a key to \a KeyNone disables it. Note that input
     * mappings do not affect text entry.
     */
     ...
     * translated into a '\a to' mouse button press before any further processing is done. Note that input mappings do not
     * affect text entry.
     */
    void setInputMapping(KeyConstant from, MouseButton to);

    /**
     * Creates an input mapping from one mouse button to another, i.e. when a raw '\a from' mouse button press is received it
     * will be translated into a '\a to' mouse button press before any further processing is done.
     */
    void setInputMapping(MouseButton from, MouseButton to);

    /**
     * Creates an input mapping from a mouse button to a key, i.e. when a raw '\a from' mouse button press is received it will
     * be translated into a '\a to' keypress before any further processing is done. Mapping a mouse button to \a KeyNone
     * disables
    ...
    /**
     * On platforms that support an onscreen keyboard such as iOS this method will show it on the screen on top of the rendered
     * scenes. Returns success flag.
     */
    ...
    /**
     * Returns whether or not the passed touch event is currently enabled, disabled events will not be detected. The valid event
     * types are TouchTapEvent, \a TouchPinchEvent, \a TouchRotationEvent, \a TouchSwipeEvent and \a TouchPanEvent, and none of
     * these are enabled by default. On devices without touchscreens this method always returns false.
     */
    ...
    /**
     * Returns whether or not the current device is a smartphone style of device such as the iPhone, iPod Touch or similar.
     */
    ...
    /**
     * Returns the number of primary CPUs on the current platform, this counts each core of a multicore processor as a separate
     * CPU.
     */
    virtual unsigned int getCPUCount() const { return 1; }

    /**
     * Returns the clock frequency of the primary CPUs on the current platform. If the CPU frequency is unknown then zero is
     * returned.
     */
    virtual uint64_t getCPUFrequency() const { return 0; }

    /**
     * On platforms that allow application control of the CPU frequency, this method can be used to throttle the CPU down or up
     * as needed, generally with the aim of giving a desirable performance/battery life tradeoff. Returns success flag.
     */
    ...
    /**
     * Returns whether Oculus Rift rendering is supported on this platform. Use PlatformInterface::isOculusRiftPresent() to
     * check for the presence of an actual connected device.
     */
     ...
     */
    virtual Matrix4 getOculusRiftProjectionMatrixLeftEye(float nearPlaneDistance, float farPlaneDistance) const { return {}; }

    /**
     * Returns the projection matrix to use for the Oculus Rift's right eye given the specified clipping plane distances.
     */
    virtual Matrix4 getOculusRiftProjectionMatrixRightEye(float nearPlaneDistance, float farPlaneDistance) const { return {}; }
...
protected:
    ...
    // Platform subclasses should call these methods when they receive raw input events in order to to make sure that input
    // mappings are correctly obeyed
