...
public:
    ...
    /**
     * Given a filename that may or may not have an extension this method tries to load a sound out of it. If the given filename
     * contains an extension then that format will be assumed, otherwise the filesystem will be searched for a matching filename
     * with an extension that has a reader function available. If one is found then it will be used to read the sound. Returns
     * success flag.
     */
    static bool loadSoundFile(const UnicodeString& filename, SoundInterface::AudioFormat& format, unsigned int& channelCount,
                              unsigned int& frequency, Vector<byte_t>& data);

    /**
     * Saves the passed sound data to a file, the format of the image file is determined by the extension present on the passed
     * filename. Returns success flag.
     */
    static bool saveSoundFile(const UnicodeString& filename, SoundInterface::AudioFormat format, unsigned int channelCount,
...
/**
 * Registers reading and writing functions for the sound file format with the given extension. If a null function pointer is
