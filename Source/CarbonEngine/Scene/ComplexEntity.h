...
public:
    ...
    /**
     * Returns the child entity at the given index cast to the specified entity type, or null if the cast is not possible.
     */
    ...
    /**
     * Returns the child entity of the specified type and with the given name, or null if such a child entity is not found.
     */
     ...
     */
    Vector<Entity*> getChildren(bool includeInternalEntities = false) { return getChildren<Entity>(includeInternalEntities); }
    ...
    /**
     * Adds an entity as a child of this complex entity, whole entity hierarchies will automatically be added in one call if the
     * passed entity is a complex entity that has children. If the passed entity is already a child of this complex entity then
     * nothing will be done and true will be returned. If the passed entity is already in the same scene as this complex entity
     * then this method will update the location in the scene graph hierarchy of the passed entity and any child entities it
     * has, this behavior allows individual entities, as well as whole entity subtrees, to be quickly and efficiently relocated
     * to a new position in the scene graph hierarchy. Returns success flag.
     */
     ...
     * with children then all the children will also be removed from the scene. If there is no hierarchy under the passed
     * entity, this complex entity is in a scene, and the passed entity was created by the engine through Scene::addEntity<>()
     * or ComplexEntity::addChild<>(), then the entity will be automatically deleted following its removal from the scene.
    ...
    /**
     * Returns whether local space culling of child entities is enabled on this complex entity, defaults to false. This can be
     * used to improve culling speed when this complex entity is being moved every frame (or very frequently) but when the
     * majority of its children do not move relative to it. Normal culling procedure would require the world space transform of
     * the child entities to be recalculated every frame in order for culling to be done, however if local space child culling
     * is enabled then the culling of child entities will instead be done by this complex entity in local space which avoids the
     * calculation of the child world space transforms. Note that this means that the culling uses the child's local extents as
     * returned by Entity::getLocalExtents() as the complex entity needs to determine the bounds around the entire entity
     * subtree rooted at each child entity in order to safely cull that branch.
     */
