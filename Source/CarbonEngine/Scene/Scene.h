...
public:
    ...
    /**
     * When a scene is loaded any entities that have a name starting with the static mesh conversion prefix are turned
     * into static meshes and placed into the scene. The prefix is stripped off to get the name of the mesh to embed.
     * Currently "mesh_".
     */
    ...
    /**
     * Clears the entire contents of this scene and reverts to default state, this internally calls
     * Scene::removeAllEntities() as part of its processing.
     */
    void clear();

    /**
     * Removes all entities from this scene, this also clears static meshes and collision/physics data that may be
     * present on this scene, but other settings are left intact.
     */
    ...
    /**
     * Returns whether this scene is 2D, 2D scenes can manage input handling and do not require a camera in them in
     * order to render, though one can be used. Scenes are not 2D by default.
     */
    ...
    /**
     * Sets the background material for this scene, the background will be drawn over the whole window if this is a 2D
     * scene. An empty string means no background material will be drawn.
     */
    ...
    /**
     * Sets whether this scene is enabled. Disabled scenes will not process input. For 2D scenes this means they will
     * not respond to user input or fire GUI events.
     */
    void setEnabled(bool enabled);

    /**
     * Returns whether this scene is visible. Defaults to true. A scene can only be drawn if it is visible, so if an
     * invisible scene is queued for rendering it will not be drawn. For a scene to be drawn it must be both visible and
     * queued for rendering using Scene::queueForRendering().
     */
     ...
     * applications that render multiple separate scenes to build up a final rendered image may want to share a depth
     * buffer between them and can do so by disabling depth clearing.
     */
    bool isDepthClearEnabled() const override { return isDepthClearEnabled_; }

    /**
     * Sets whether a depth clear will be done prior to rendering this scene, for more details see
     * Scene::isDepthClearEnabled().
     */
    void setDepthClearEnabled(bool enabled) { isDepthClearEnabled_ = enabled; }

    /**
     * Returns whether depth testing will be enabled when rendering this scene, currently depth testing is enabled for
     * 3D scenes and disabled for 2D scenes.
     */
    bool isDepthTestEnabled() const override { return is3D(); }

    /**
     * Adds an existing entity as a child of the root entity of this scene. Returns success flag. To add an entity as a
     * child of an existing entity use ComplexEntity::addChild().
     */
    bool addEntity(Entity* entity);

    /**
     * Creates a new entity of the specified type and adds it to this scene, returning the new entity instance. The name
     * of the new entity is specified, and the new entity's `initialize()` method will be called with any additional
     * arguments that are passed.
     */
    template <typename EntityType, typename... ArgTypes> EntityType* addEntity(const String& name, ArgTypes&&... args)
    {
    ...
    /**
     * Returns the entity in this scene with the given name and type, or null if one is not found. If \a name is an
     * empty string then name checking is not performed and only the type requirement will be enforced.
     */
    ...
    /**
     * For use in 2D scenes, this returns the orthographic viewing rectangle of the default camera, based on the
     * camera's position and the return value from Camera::getOrthographicSize(). If there is no camera in the 2D scene
     * then the viewport rect is returned, which is equivalent to a camera on the origin with an orthographic size of
     * the main window's width and height.
     */
    Rect getDefaultCameraOrthographicRect() const;

    /**
     * This is a helper method for setting up a 2D orthographic camera in this scene with the specified orthographic
     * width and height, see Camera::setOrthographicSize() for details on how these two values are interpreted. This
     * method automatically calls Scene::setIs2D(true). The created Camera instance is returned for use by the
     * application and will be automatically cleaned up along with the scene.
     */
    ...
    /**
     * If this is a 2D scene then one of the GUIWindow entities in the scene can potentially have focus. This returns
     * the window with focus or null if no window currently has focus.
     */
    GUIWindow* getFocusWindow() { return focusWindow_; }

    /**
     * Sets the GUIWindow entity that currently has focus. Calling this method with an argument of null causes no window
     * to have focus. Appropriate GUIWindowLoseFocus and GUIWindowGainFocus events will be sent. Returns success flag.
     */
    ...
    /**
     * Rewrites all .mesh files that are stored as embedded resources in this scene, this just involves loading and then
     * saving them back to the same resource which can be useful when upgrading to newer file formats. Embedded meshes
     * are how scenes store their geometry. Returns success flag.
     */
    bool rewriteEmbeddedMeshes();

    /**
     * Returns whether or not geometry with the specified material can be considered to be part of the world geometry.
     * By default all materials will return true from this method, however if Scene::addWorldGeometryMaterial() is used
     * then only those materials added using that method will return true from this method. Note that world geometry
     * must first be flagged as such by using Entity::setIsWorldGeometry().
     */
    bool isWorldGeometryMaterial(const String& materialName) const;

    /**
     * Adds the specified material to the list of allowed world geometry materials. By default this list is empty, which
     * means all geometry can be considered to be world geometry regardless of its material. Once this list has been
     * started, only geometry that uses one of the materials added to it can be considered part of the world geometry
     * for this scene. This can be used to customize what is and what isn't treated as world geometry when doing things
     * such as intersection testing via Scene::intersect(). Note that world geometry must first be flagged as such by
     * using Entity::setWorldGeometry().
     */
    ...
    /**
     * Intersects a ray with this scene. If there was an intersection then the details of the closest intersection are
     * returned. If no intersection occurred then the return value is an empty IntersectionResult that will evaluate to
     * false. If \a onlyWorldGeometry is true then only entities flagged as world geometry will be tested for
     * intersection.
     */
    IntersectionResult intersect(const Ray& ray, bool onlyWorldGeometry = false);

    /**
     * Intersects a ray with this scene, where the ray is cast through the given screen pixel of the given camera. If no
     * camera is given then the first camera in the scene is used. If there was an intersection then the details of the
     * closest intersection are returned. If no intersection occurred then the return value is an empty
     * IntersectionResult that will evaluate to false. If \a onlyWorldGeometry is true then only entities flagged as
     * world geometry will be tested for intersection.
     */
    IntersectionResult intersect(const Vec2& pixel, Camera* camera = nullptr, bool onlyWorldGeometry = false);

    /**
     * Intersects a ray with this scene. All the intersection results are returned in the \a results vector sorted from
     * nearest to farthest. The return value indicates whether any intersections were detected. If \a onlyWorldGeometry
     * is true then only entities flagged as world geometry will be tested for intersection.
     */
    bool intersect(const Ray& ray, Vector<IntersectionResult>& intersections, bool onlyWorldGeometry = false);

    /**
     * Intersects a ray with this scene, where the ray is cast through the given screen pixel of the given camera. If no
     * camera is given then the first camera in the scene is used. All the intersection results are returned in the \a
     * results vector sorted from nearest to farthest. The return value indicates whether any intersections were
     * detected. If \a onlyWorldGeometry is true then only entities flagged as world geometry will be tested for
     * intersection.
     */
    ...
    /**
     * Ensures all data the scene needs to render is ready to go so there will be no stuttering as new parts of the
     * scene come into view.
     */
    void precache();

    /**
     * Returns whether the contents of the scene is internally consistent. This does a number of sanity and correctness
     * checks on the scene graph and reports any problems it finds as warnings. The return value indicates whether any
     * problems were found.
     */
    ...
    /**
     * Sets the current post-process materials being used when rendering this scene. If multiple post-process materials
     * are specified then they are applied consecutively.
     */
    ...
    /**
     * Sets whether post-process pass-through is enabled for this scene, see Scene::isPostProcessPassThroughEnabled()
     * for details.
     */
    void setPostProcessPassThroughEnabled(bool enabled) { isPostProcessPassThroughEnabled_ = enabled; }

    /**
     * For 2D scenes, this adds an offset to the root entity so that all the GUI elements in the scene are centered in
     * the window.
     */
    void center();
    ...
    /**
     * Sets up the passed FileReader instance to read from the embedded resource with the given name. Returns success
     * flag.
     */
    bool getEmbeddedResource(const UnicodeString& name, FileReader& file) const;

    /**
     * Adds an embedded resource to this scene. If there is already an embedded resource with the given name then false
     * is returned. Returns success flag.
     */
    ...
    /**
     * Returns the current root that is prepended to all static meshes added with Scene::addStaticMesh(). This allows
     * easy scoping of all static meshes to a single directory. Defaults to "Static/".
     */
    ...
    /**
     * This method is intended for use when this scene is being saved and one of its entities has an Entity pointer that
     * it needs to persist. The pointer can't be written directly, so this method must be used to save a bit of data
     * that will allow the entity pointer to be re-established when the scene is loaded. This should be used with
    ...
    /**
     * Queues a scene for rendering with the given camera. If \a camera is null then the first camera found in the scene
     * will be used.
     */
    void queueForRendering(const Camera* camera = nullptr, int priority = 0);

    /**
     * Renders this scene into the given 2D texture using the given camera. If \a camera is null then the first camera
     * found in the scene will be used. This method requires that the graphics hardware supports offscreen render
     * targets, support for this feature can be queried using GraphicsInterface::isRenderTargetSupported(). Returns
     * success flag.
     */
    bool renderToTexture(Texture2D* texture, Camera* camera = nullptr);

    /**
     * Renders this scene into the given cubemap texture using the given camera. This method is only supported on 3D
     * scenes. If \a camera is null then the first camera found in the scene will be used. This method requires that the
     * graphics hardware supports cubemaps and offscreen render targets. Returns success flag.
     */
    ...
    /**
     * Sets whether deferred lighting should be used when rendering this scene. Renderer::isDeferredLightingSupported()
     * can be used to check for hardware support if desired. Defaults to false.
     */
    void setDeferredLightingEnabled(bool enabled) { isDeferredLightingEnabled_ = enabled; }

    /**
     * Adds a colored line to this scene's immediate geometry. Immediate geometry is simple colored geometry that is
     * rendered as part of the scene but isn't stored in a separate entity. It is often convenient when debugging but
    ...
    /**
     * Adds the passed AABB to this scene's immediate geometry, internally this adds the edges of the AABB as individual
     * lines.
     */
     ...
     */
    void addImmediateGeometry(const Vector<Vec3>& vertices,
                              const SimpleTransform& transform = SimpleTransform::Identity,
                              const Color& color = Color::White);
    ...
    /**
     * Causes the given texture to stay loaded regardless of whether any entities are holding a reference to it or not.
     * This effectively causes this scene to hold a reference to the given texture which ensures that it will not be
     * unloaded during the lifecycle of this scene. Texture references held by a scene can be released using
     * Scene::releasePrecachedTexture() or Scene::releasePrecachedTextures(), and all texture references held by the
     * scene are automatically cleaned up on scene shutdown. Returns success flag.
     */
    ...
    /**
     * Returns a list of all the entities in this scene that are of the specified type. The entities are returned in the
     * passed \a entities vector.
     */
    ...
    /**
     * Proxies through to Camera::screenToWorld() on this scene's default camera, or returns the point unchanged if this
    ...
    /**
     * Returns the entity with the given name in the scene with the given name, or null if there is no scene or entity
    ...
    /**
     * Returns the post-process materials that are being applied globally to the combined output of all rendered scenes.
     * If multiple post-process materials are specified then they are applied consecutively.
     */
    static const Vector<String>& getGlobalPostProcessMaterials() { return globalPostProcessMaterials_; }

    /**
     * Sets the post-process materials to apply globally to the combined output of all rendered scenes. If multiple
     * post-process materials are specified then they are applied consecutively.
     */
    static void setGlobalPostProcessMaterials(const Vector<String>& materials);

    /**
     * Adds a material to use for global post-processing on the combined output of all rendered scenes, the new material
     * is appended to the existing list of global post-process materials.
     */
    ...
    /**
     * The supported Oculus Rift modes, the Rift can be disabled, enabled alongside the default rendering output which
     * is useful when testing, or rendered to exclusively with no output shown on the default graphics output.
     */
    enum OculusRiftMode
    {
        OculusRiftDisabled,
...
private:
    ...
    // These entities require Entity::update() to be called every frame, see Entity::isPerFrameUpdateRequired() for
    // details
    ...
    // Used to restore entity references when a scene is being loaded from file. Entity references are stored as indices
    // into this array. See Scene::saveEntityReference() and Scene::loadEntityReference() for details.
    ...
    // Embedded resources are stored in a simple file system volume so they can be easily made accessible through the
    // virtual file system
