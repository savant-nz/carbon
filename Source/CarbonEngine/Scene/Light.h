...
public:
    ...
    enum LightType
    {
        ...
        /**
         * A directional light that simulates parallel light rays coming from a source at infinity. It has only
         * direction and no position.
         */
        DirectionalLight,

        /**
         * A point light that emits light equally in all directions. It has a position, and direction is only relevant
         * when using a projection texture on the light.
         */
        PointLight,

        /**
         * A spot light that emits light in a specific cone down the light's world space Z direction. The size of the
         * spotlight cone can be set using Light::setMinimumConeAngle() and Light::setMaximumConeAngle().
         */
        SpotLight,
    ...
    /**
     * For spot lights, this returns the angle away from the Z axis at which cone attenuation will start occurring. This
     * value should not be greater than the maximum cone angle, and can't be greater than half Pi. Defaults to Pi / 8.
     */
    ...
    /**
     * For spot lights, this returns the angle away from the Z axis at which cone attenuation will finish, i.e. there
     * will be no illumination outside this angle. This value should not be less than the minimum cone angle, and can't
     * be greater than half Pi. Defaults to Pi / 4.
     */
    ...
    /**
     * Returns the name of the 2D texture currently being projected by this light. This is projected out from positional
     * light sources along the Z axis of this light. The maximum cone angle (see Light::getMaximumConeAngle()) can be
     * used to control the size of the 2D projection.
     */
    ...
    /**
     * Returns the name of the projection cubemap texture currently being used on this light. Light projection cubemaps
     * are projected out from positional light sources onto the surrounding scene and take the light's orientation into
     * account. Only available on point lights and spot lights. Note that it is possible to simulate spot lights using a
     * point light with an appropriate projection cubemap applied to it. If this name is blank then no projection
     * cubemap will be used on this light.
     */
    ...
    /**
     * Returns whether specular highlights should be computed when rendering this light source. Defaults to false.
     * Turning specular highlights off may increase rendering performance in applications that are fragment processor
     * bound. They may also be turned off for artistic reasons.
     */
    bool isSpecularEnabled() const override { return isSpecularEnabled_; }

    /**
     * Sets whether specular lighting should be computed when rendering this light, see Light::isSpecularEnabled() for
     * details.
     */
    void setSpecularEnabled(bool enabled) { isSpecularEnabled_ = enabled; }

    /**
     * Returns the current intensity of the specular highlights cast by this light, this is only used when specular is
     * enabled on this light. Defaults to 1.0,
     */
    ...
    /**
     * Shorthand method for setting this light up as an ambient light. This method sets the light type to \a
     * AmbientLight and sets the light color.
     */
    ...
    /**
     * Shorthand method for setting this light up as a point light. This method sets the light type to \a PointLight,
     * sets the light position, and sets the light color.
     */
    void setPointLight(const Color& color, const Vec3& position, float radius);

    /**
     * Shorthand method for setting this light up as a spot light. This method sets the light type to \a SpotLight, sets
     * the light color, sets the light radius, and sets the light direction.
     */
