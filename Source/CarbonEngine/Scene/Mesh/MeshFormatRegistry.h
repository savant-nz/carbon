...
public:
    ...
    /**
     * Given a filename that may or may not have an extension this method tries to load a mesh out of it. If the give filename
     * contains an extension then that format will be assumed, otherwise the filesystem will be searched for a matching filename
     * with an extension that has a reader function available. If one is found then it will be used to read the mesh. Returns
     * success flag.
     */
     ...
     * filename. Note that some supported mesh formats do not support all of the features of the Mesh class, and so certain
     * parts of the mesh data may be skipped depending on the target format. The only mesh format which is guaranteed to be able
     * to persist a Mesh instance without losing any data is the native '.mesh' format, this is because the '.mesh' format is a
     * memory dump of the entire contents of a Mesh instance. However, because it is a native format, '.mesh' files are not
     * supported by other mesh viewing or editing applications. Returns success flag.
     */
...
/**
 * Registers reading and writing functions for the mesh file format with the given extension. If a null function pointer is
