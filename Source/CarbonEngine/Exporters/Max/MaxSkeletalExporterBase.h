...
protected:
    ...
    /**
     * This is called for each physique found by SkeletalExporterBase::exportData(). Should be implemented by subclasses
     * to get out the data they are interested in. This will be called for every physique found, even if only a subset
     * of physiques are selected and the 'Export Selected' option os chosen, in this case the \a doExport parameter
     * describes whether or not the data in this physique should actually be exported.
     */
    virtual bool exportPhysique(INode* node, IPhysiqueExport* phy, IPhyContextExport* mcExport, bool doExport) = 0;

    /**
     * Same as SkeletalExporterBase::exportPhysique() but for ISkin nodes. The skin modifier for the node is disabled
     * before this method is called and then re-enabled afterwards.
     */
    virtual bool exportSkin(INode* node, ISkin* skin, ISkinContextData* skinContext, bool doExport) = 0;

    /**
     * Looks through the scene for physiques and skins. If only physiques are found then
     * SkeletalExporterBase::exportPhysique() is called for each one of them and likewise for skins. If both physiques
     * and skins are found then a warning is logged and only the physiques will be exported. This is to avoid potential
     * confusion from trying to export both physiques and skins at the same time.
     */
    bool exportData(Runnable& r);

    /**
     * If the given node is a bone then it will be added to the bones list and a bone index returned. Returns -1 on
     * failure.
     */
