...
String Helper::processMaterialName(const String& name)
{
    ...
    // Maya doesn't allow forward slashes in shader names, so two consecutive underscores are replaced with a forward
    // slash here to allow forward slashes in exported material names
    result.replace("__", "/");
...
String Helper::getTextureOutputName(MObject node)
{
    ...
    // If the name of the file texture is one autogenerated by Maya (of type "file<number>") then instead use the name
    // of the original texture file that was used
