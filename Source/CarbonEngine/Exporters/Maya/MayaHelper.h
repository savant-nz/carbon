...
public:
    ...
    /**
     * Returns all the objects of the given type that should be exported. If Maya::onlyExportSelected is true then only
     * the selected objects of the given type are returned.
     */
    ...
    /**
     * This method is used in tandem with the arrays returned by MFnMesh::getConnectedShaders() and returns the material
     * name for the polygon at the given index, based on the \a shaders and \a indices arrays provided by that method.
     * This also handles falling back to MaterialManager::ExporterNoMaterialFallback if no material is specified for a
     * polygon and will replace two consecutive underscores in a material name with a forward slash. This provides a way
     * to specify a '/' in a material name that gets around Maya not allowing special characters.
     */
    static String getMaterialName(int polygonIndex, const MObjectArray& shaders, const MIntArray& indices);

    /**
     * Exports all materials referenced by the triangle set as material files in the given directory. Some material
     * properties from Maya are preserved where possible. Does not overwrite any existing files.
     */
