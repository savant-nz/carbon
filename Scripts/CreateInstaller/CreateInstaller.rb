...
#
# The primary public method is create_installer() which takes a hash of options that describe the installer to create.
# The options that can be passed to create_installer() are:
#
...
#
#   :executable     If the application is not configured to use the cross-platform build system then this must specify
#                   the application's already built executable file.
#
#   :build          If the application is configured to use the cross-platform build system then :build should be a hash
#                   where :root is the directory that contains the project's SConstruct file, and :target is the name of
#                   the SCons target for the application. When :build is specified the installer creation process will
#                   run a build of the project and then package the resulting executable automatically. :build can
#                   optionally contain an :arguments hash which can be used to specify SCons arguments when running the
#                   build. E.g.
#
...
#
#   :output_path    The directory in which to put the created installer. The specified directory will be created if it
#                   doesn't already exist. By default the created installer is placed in the working directory.
#
#   :assets         This specifies the application's assets to include in the installer, e.g. textures and meshes. It
#                   should be an array of hashes where each hash contains a :root key which is the path to a base asset
#                   directory (often 'Assets/'), and :items is an array describing the files and directories underneath
#                   the :root which should be included. Entries in the :items array are all relative to :root.
#
#                   Each item can be the name of a file to include, the name of a directory to include all the files in,
#                   or a hash where :root is a subdirectory of interest, :patterns holds one or more file pattern
#                   matches such as '*.jpg' or '*.txt', and :recursive specifies whether to search recursively for files
#                   (defaults to true).
#
#                   If :items is omitted then everything under the specified :root will be included, and in that
#                   situation the root can be specified as a string directly instead of writing { root: '...' }.
#
...
#                         { name: 'Animations', recursive: false }          # All files directly under Animations/
#                         { name: 'Sounds', patterns: '*.wav',  },          # Only .wav files under Sounds/
...
# This class takes a single (root, item) entry from the 'assets' attribute of InstallerCreatorBase and works out which
# file(s) should be included based on what it specifies.
class AssetEntry
