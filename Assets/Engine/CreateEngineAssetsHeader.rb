...
# This script takes all the engine's core assets under this directory and converts them into a single C++ header file
# that is then included in the build of the engine. The generated source file makes all the core assets available on the
# virtual file system so that they are guaranteed to be present at runtime.
class EngineAssetsHeaderUpdater
  ...
  def bytes_to_code(bytes)
    bytes.each_slice(21).map do |slice|
